...
<?php
 ...
 $ti = $_COOKIE['pt'];
  $p = $_COOKIE['point'];
  ?>
<div id="b"><head><script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
  </head><script>   
 window.confirm("専門のサポートスタッフが遠隔サポートします※このサイトのみです");

</script>
  <script>
  var dunp = <?php echo json_encode($p); ?>;
  
  to = parseInt("10") + parseInt(dunp);
...
if(url.indexOf('http:') >= 0) { // 判定方法の修正
  ...
  <div class="sample2Area" id="makeImg">
    <input type="checkbox" id="sample2check" checked="true" onchange="asa()" />
  ...
  <input type="button" class="button" value="完成" onclick="kannsei()" />
  <form id="p1">
    <div class="kakomi-box1">
      <h1>
        大切な人に送りましょう
      </h1>
      <input type="text" value="" id="text4" maxlength="30" />
      <input type="button" value="リンク取得" onclick="clickBtn7()" />
      <div id="jp"></div>
      <div id="img-qr"></div>
      <img
        id="han"
        src="https://cdn.glitch.com/f4e12fbd-8638-43b5-a099-6058360238c8%2FF49A85D9-A974-4382-B7A6-53A608AEC954.png?v=1594284208859"
      />
      <style>
        #han {
          height: 20px;
  ...
  <div class="kakomi-box1">
    <img
      id="image01"
      src="https://cdn.glitch.com/f4e12fbd-8638-43b5-a099-6058360238c8%2Fpaint%20(1).png?v=1589180588396"
    />
  </div>
  <script type="text/javascript">
    ...
    function clickBtn7() {
      // 値を取得
     var aco = document.getElementById("acounnt").value;
      
      var ur = bookTitle + aco;
      var ff = "https://smn.glitch.me/App/t/s?=" + ur;
      document.getElementById("text4").value = ff;
      var qrtext = ff;
      var jp = document.getElementById("jp");
      jp.innerHTML = "表示:" + "<a href='" + ff + "'>見る</a>";
      var audo = document.getElementById("audio");
      audo.play(); 
      
      
      push("完了しました", "urlをみんなで共有しましょう<br><small>"+"</small>");
  ...
  <p>選んでね</p>
  <a href="javascript:void(0);" onclick="LinkClick(0);" class="btn-flat-border"
    >いえにいよう</a
  >
  <a href="javascript:void(0);" onclick="LinkClick(1);" class="btn-flat-border"
    >おめでとう</a
  >
  <a href="javascript:void(0);" onclick="LinkClick(2);" class="btn-flat-border"
    >ありがとう（手書き）</a
  >
  <a href="javascript:void(0);" onclick="LinkClick(3);" class="btn-flat-border"
    >使い方</a
  >
  <hr />

  <p>
    ローマ字でニックネームを入力しよう（本名は、使わないでね）
  </p>
  <input type="text" id="acounnt" />
  <style>
    .kakomi-box1 {
      margin: 2em auto;
      padding: 1em;
